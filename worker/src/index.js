...
export default {
  async fetch(request, env) {
    ...
    if (request.method === "GET" && url.pathname === "/health") return json({ ok: true, service: "trading-reels-ai-worker" });
    if (request.method === "GET" && url.pathname === "/api/media-status") {
      return json({
        ok: true,
        pexelsConfigured: Boolean(env.PEXELS_API_KEY),
        pixabayConfigured: Boolean(env.PIXABAY_API_KEY),
        mode: "metadata-only",
      });
    }
    if (request.method === "POST" && url.pathname === "/api/explain") return handleExplain(request, env);
    if (request.method === "POST" && url.pathname === "/api/market-candles") return handleMarketCandles(request, env);
...
async function handleExplain(request, env) {
  ...
  if (env.GEMINI_API_KEY) {
    const gemini = await callGemini(env.GEMINI_API_KEY, prompt).catch(() => null);
    if (gemini) return json({ ...parseAiText(gemini, fallback), provider: "gemini" });
  }

  if (env.OPENROUTER_API_KEY) {
    const openrouter = await callOpenRouter(env.OPENROUTER_API_KEY, prompt).catch(() => null);
    if (openrouter) return json({ ...parseAiText(openrouter, fallback), provider: "openrouter" });
  }

  return json({ ...fallback, provider: "fallback" });
}

async function handleMarketCandles(request, env) {
  let body = {};
  try {
    body = await request.json();
  } catch {
    return json(marketFallback(), 400);
  }

  const input = normalizeMarketInput(body);
  const prefer = input.provider;

  if ((prefer === "auto" || prefer === "twelvedata") && env.TWELVE_DATA_API_KEY) {
    const candles = await fetchTwelveDataCandles(env.TWELVE_DATA_API_KEY, input).catch(() => []);
    if (candles.length) return json({ provider: "twelvedata", symbol: input.symbol, interval: input.interval, candles, fallback: false });
  }

  if ((prefer === "auto" || prefer === "alphavantage") && env.ALPHA_VANTAGE_API_KEY) {
    const candles = await fetchAlphaVantageCandles(env.ALPHA_VANTAGE_API_KEY, input).catch(() => []);
    if (candles.length) return json({ provider: "alphavantage", symbol: input.symbol, interval: input.interval, candles, fallback: false });
  }

  return json(marketFallback(input));
}
...
function normalizeMarketInput(body) {
  return {
    ...
    provider: ["auto", "twelvedata", "alphavantage"].includes(cleanText(body.provider).toLowerCase()) ? cleanText(body.provider).toLowerCase() : "auto",
  };
}
